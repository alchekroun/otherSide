...
public:
    explicit DisplayedMessagePanel(PeerId source, std::shared_ptr<DisplayMesssageFeed> feed) : _feed(feed), _source(source) {}
    ...
    void render() override {
        ...
        for (const auto& msg : _feed->getMessages()) {
            ...
            ImGui::TextWrapped(
                "[%s][%s] %s",
                formatTime(msg.createdTs).c_str(),
                peerIdToString[msg.from].c_str(),
                msg.text.c_str()
            );
            ImGui::PopStyleColor();
        }
...
private:
    ...
    std::unordered_map<PeerId, std::string> peerIdToString = {
        { PeerId::HOST, "Host" },
